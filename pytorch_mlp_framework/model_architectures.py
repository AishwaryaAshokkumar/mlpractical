...
class ConvolutionalProcessingBlock_BN(nn.Module):
    ...
    def build_module(self):
        ...
        out = self.layer_dict['conv_0'].forward(out)
        self.layer_dict['conv_0_bn'] = nn.BatchNorm2d(self.num_filters)
        out = self.layer_dict['conv_0_bn'].forward(out)
        out = F.leaky_relu(out)
        ...
        out = self.layer_dict['conv_1'].forward(out)
        self.layer_dict['conv_1_bn'] = nn.BatchNorm2d(self.num_filters)
        out = self.layer_dict['conv_1_bn'].forward(out)
        out = F.leaky_relu(out)
